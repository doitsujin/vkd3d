...
void test_depth_bias_behaviour(void)
{
    ...
    float4 main(in uint id : SV_VERTEXID) : SV_POSITION
    {
        ...
        return float4(coords * float2(2, -2) + float2(-1, 1), 0.5f + 0.25f * coords.y, 1);
    }
#endif
    static const DWORD vs_code[] =
    {
        0x43425844, 0xcb13b3eb, 0xcd69d4cd, 0xf7ae88bc, 0xd86457b2, 0x00000001, 0x00000180, 0x00000003,
        0x0000002c, 0x00000060, 0x00000094, 0x4e475349, 0x0000002c, 0x00000001, 0x00000008, 0x00000020,
        0x00000000, 0x00000006, 0x00000001, 0x00000000, 0x00000101, 0x565f5653, 0x45545245, 0x00444958,
        0x4e47534f, 0x0000002c, 0x00000001, 0x00000008, 0x00000020, 0x00000000, 0x00000001, 0x00000003,
        0x00000000, 0x0000000f, 0x505f5653, 0x5449534f, 0x004e4f49, 0x58454853, 0x000000e4, 0x00010050,
        0x00000039, 0x0100086a, 0x04000060, 0x00101012, 0x00000000, 0x00000006, 0x04000067, 0x001020f2,
        ...
        0x00100082, 0x00000000, 0x0010100a, 0x00000000, 0x00004001, 0x00000002, 0x05000056, 0x00100072,
        0x00000000, 0x001003c6, 0x00000000, 0x0f000032, 0x00102072, 0x00000000, 0x00100246, 0x00000000,
        0x00004002, 0x40000000, 0xc0000000, 0x3e800000, 0x00000000, 0x00004002, 0xbf800000, 0x3f800000,
        0x3f000000, 0x00000000, 0x05000036, 0x00102082, 0x00000000, 0x00004001, 0x3f800000, 0x0100003e,
    };
    ...
    for (i = 0; i < ARRAY_SIZE(tests); i++)
    {
        for (j = 0; j < (options16.DynamicDepthBiasSupported ? 2 : 1); j++)
        {
            ...
            pso_desc.Flags = j ? D3D12_PIPELINE_STATE_FLAG_DYNAMIC_DEPTH_BIAS : 0u;
            ...
            for (y = 0; y < 4; y++)
            {
                for (x = 0; x < 4; x++)
                {
                    ...
                    uint32_t ref = get_readback_uint(&rb_no_bias, x, y, 0);

                    todo_if(!tests[i].dsv_handle && tests[i].depth_bias_constant && is_amd_vulkan_device(context.device))
                    ok(got > ref && got <= 0x3f800000u, "Got %#x, reference is %#x at (%u,%u).\n", got, ref, x, y);
                }
            }
